...
const sassGlob = require('gulp-sass-glob'); //групповой импорт файлов со стилями Sass (по маске файлов)
...
const uglify = require('gulp-uglify'); //убирает из текста комментарии и пустые строки, сокращает имена переменных, код записывается в одну строку. Неиспользуемый код удаляется (операция tree shaking).
